...
/**
 * Generate a facility name from sport name and existing facilities count.
 * Example: If sportName is "Tenis" and there's already 1 tennis facility, returns "Tenis 2"
 */
function generateFacilityName(
  sportName: string,
  sportId: number,
  existingFacilities: CollectedFacility[]
): string {
  // Count how many facilities with the same sport already exist
  const sameSpportCount = existingFacilities.filter((f) => f.sportId === sportId).length;
  return `${sportName} ${sameSpportCount + 1}`;
}
...
export const conversationRepository = {
  ...
  async addFacility(
    id: string,
    facility: Omit<CollectedFacility, 'name'> & { name?: string }
  ): Promise<Conversation | null> {
    // Validate schedules
    ...
    return this.updateCollectedData(id, (current) => {
      ...
      const facilityName =
        facility.name?.trim() ||
        generateFacilityName(facility.sportName, facility.sportId, existingFacilities);

      const completeFacility: CollectedFacility = {
        ...facility,
        name: facilityName,
      };
      ...
      if (existingIndex >= 0) {
        ...
        updatedFacilities[existingIndex] = completeFacility;
        return {
          ...current,
      ...
      // Add new facility
      logger.info({ id, facilityName, autoGenerated: !facility.name?.trim() }, 'Adding facility');
      return {
        ...current,
        facilities: [...existingFacilities, completeFacility],
      };
    });
  },
